...
- [x] 2. 扩展 trakt.ts：实现带缓存的 getShowDetail / getSeasons / getEpisodes
  - [x] 2.1 实现 `getShowDetail(traktId, userId)` 方法
  ...
  - [x] 2.4 实现 `getSeasons(traktId, userId)` 方法
  ...
  - [x] 2.6 实现 `getEpisodes(traktId, seasonNumber, userId)` 方法
...
- [x] 3. Checkpoint — 确认 trakt.ts 扩展完整
  - 确保所有测试通过，如有疑问请询问用户。

- [x] 4. 重构 sync.ts：实现 upsertShowFromTrakt
  - [x] 4.1 实现 `upsertShowFromTrakt(traktId, traktShow, userId)` 函数骨架
  ...
  - [x] 4.3 实现 seasons 和 episodes 并发 upsert
  ...
  - [x]* 4.4 为 upsertShowFromTrakt 编写属性测试：tmdbId 非空写入
  ...
  - [x]* 4.5 为 upsertShowFromTrakt 编写属性测试：TBA 字段为 null
...
- [x] 5. 重构 sync.ts：替换所有 upsertShowFromTmdb 调用点
  - [x] 5.1 替换 `syncSingleShow` 中的调用
  ...
  - [x] 5.2 替换 `triggerIncrementalSync` 中的调用
...
- [x] 6. 重构 sync.ts：更新 findOrCreateEpisode 使用 Trakt
