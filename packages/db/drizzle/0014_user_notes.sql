...
-- One note per user+show+season+episode combo (episode-level)
CREATE UNIQUE INDEX "user_notes_episode_idx"
    ON "user_notes" ("user_id","show_id","season","episode")
    WHERE (season IS NOT NULL AND episode IS NOT NULL);
-- One note per user+show at show level (no season/episode)
CREATE UNIQUE INDEX "user_notes_show_idx"
    ON "user_notes" ("user_id","show_id")
    WHERE (season IS NULL AND episode IS NULL AND show_id IS NOT NULL);
-- One note per user+movie
CREATE UNIQUE INDEX "user_notes_movie_idx"
