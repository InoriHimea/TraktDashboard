...
async function ensureSyncState(userId) {
    await getDb().insert(syncState).values({ userId }).onConflictDoNothing();
}
// ─── Full sync ────────────────────────────────────────────────────────────────
export async function triggerFullSync(userId) {
    const db = getDb();
    ...
    try {
        ...
        await Promise.all(watchedShows.map((ws) => limit(async () => {
            ...
            try {
                await withTimeout(syncSingleShow(userId, {
                    tmdbId,
                    traktId: ws.show.ids.trakt || null,
                    title,
                    traktShow: ws.show,
                }), SHOW_TIMEOUT_MS, title);
            }
            catch (e) {
...
// ─── Incremental sync ─────────────────────────────────────────────────────────
export async function triggerIncrementalSync(userId) {
    const db = getDb();
    await ensureSyncState(userId);
...
// ─── Single show sync ─────────────────────────────────────────────────────────
async function syncSingleShow(userId, input) {
    if (!input.traktId)
        throw new Error("Missing Trakt id");
    const trakt = getTraktClient();
...
async function upsertShowFromTrakt(traktId, traktShow, userId) {
    const db = getDb();
    const trakt = getTraktClient();
    ...
    await Promise.all(traktSeasons.map((s) => seasonLimit(async () => {
        ...
        for (const ep of traktEpisodes) {
            ...
            const translatedEpTitle = tmdbEp?.translatedTitle &&
                tmdbEp.translatedTitle !== ep.title
                ? tmdbEp.translatedTitle
                : null;
            const translatedEpOverview = tmdbEp?.translatedOverview &&
...
async function upsertShowFromTmdb(tmdbId, traktShow, userId) {
    const db = getDb();
    ...
    await Promise.all(validSeasons.map((s) => seasonLimit(async () => {
        ...
        try {
            ...
            for (const ep of seasonData.episodes || []) {
                ...
                const translatedEpTitle = displayLanguage &&
                    ep.name?.trim() &&
                    ep.name !== fallbackEp?.name
                    ? ep.name.trim()
                    : null;
                const originalOverview = fallbackEp?.overview?.trim() ||
                    ep.overview?.trim() ||
                    null;
                const translatedEpOverview = displayLanguage &&
                    ep.overview?.trim() &&
...
// ─── Episode progress ─────────────────────────────────────────────────────────
async function syncEpisodeProgress(userId, showId, tmdbId, progress) {
    const db = getDb();
    for (const season of progress.seasons || []) {
        for (const ep of season.episodes || []) {
            if (!ep.completed || !ep.last_watched_at)
                continue;
            ...
            const [existing] = await db
                .select()
                .from(watchHistory)
                .where(and(eq(watchHistory.userId, userId), eq(watchHistory.episodeId, episode.id), eq(watchHistory.watchedAt, watchedAt)));
            if (!existing) {
...
async function findOrCreateEpisode(showId, tmdbId, seasonNum, episodeNum, language, userId) {
    const db = getDb();
    const [ep] = await db
        .select()
        .from(episodes)
        .where(and(eq(episodes.showId, showId), eq(episodes.seasonNumber, seasonNum), eq(episodes.episodeNumber, episodeNum)));
    if (ep)
        return ep;
...
export async function computeWatchedEpisodes(userId, showId) {
    const db = getDb();
    ...
    if (resetAt) {
        whereConditions.push(or(gt(watchHistory.watchedAt, resetAt), isNull(watchHistory.watchedAt)));
    }
    const [{ count }] = await db
        .select({
        count: sql `count(distinct ${watchHistory.episodeId})`,
    })
        .from(watchHistory)
...
// ─── Progress recalc ──────────────────────────────────────────────────────────
export async function recalcShowProgress(userId, showId) {
    const db = getDb();
    ...
    const watchedIds = (await db
        ...
        .from(watchHistory)
        .innerJoin(episodes, eq(watchHistory.episodeId, episodes.id))
        .where(and(eq(watchHistory.userId, userId), eq(episodes.showId, showId)))).map((r) => r.id);
    let nextEpisodeId = null;
    if (watchedIds.length > 0) {
        const [next] = await db
            .select({ id: episodes.id })
            .from(episodes)
            .where(and(eq(episodes.showId, showId), sql `id NOT IN (${sql.join(watchedIds.map((id) => sql `${id}`), sql `, `)})`, sql `air_date IS NOT NULL AND air_date <= ${today}`))
