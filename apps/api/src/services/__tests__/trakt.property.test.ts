...
function makeShowDetail(traktId: number) {
  return {
    title: 'Test Show',
    year: 2020,
    overview: 'Overview',
...
function makeSeasonDetail(number: number) {
  return {
    number,
    episode_count: 10,
...
function makeEpisodeDetail(epNumber: number, firstAired: string | null = '2020-01-01T00:00:00.000Z') {
  return {
    number: epNumber,
    season: 1,
    ...
    it('getEpisodes throws for any 4xx/5xx status', async () => {
      await fc.assert(
        fc.asyncProperty(
          ...
          fc.integer({ min: 400, max: 599 }),
          async (traktId, seasonNum, statusCode) => {
            mockDb.select = mock(() => ({
              from: mock(() => ({
                where: mock(() => Promise.resolve([makeMockUser()])),
              })),
