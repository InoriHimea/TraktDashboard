...
// ─── Helpers ──────────────────────────────────────────────────────────────────
function makeTraktDetail(tmdbId, traktId = 1) {
    return {
        title: 'Test Show',
        year: 2020,
...
function makeTraktSeason(number) {
    return {
        number,
        episode_count: 2,
        first_aired: '2020-01-01T00:00:00.000Z',
...
function makeTraktEpisode(epNumber, firstAired = '2020-01-01T00:00:00.000Z') {
    return {
        number: epNumber,
        season: 1,
        ...
        insert: mock((table) => ({
            values: mock((v) => {
                if (table === 'shows_table')
                    upsertedShows.push(v);
                if (table === 'episodes_table')
                    upsertedEpisodes.push(v);
                return {
...
describe('Sync Service - Property Tests (pure logic)', () => {
    ...
    describe('Property 7: upsertShowFromTrakt writes non-null tmdbId', () => {
        it('shows.tmdbId equals traktDetail.ids.tmdb for any valid tmdbId', () => {
            fc.assert(fc.property(fc.integer({ min: 1, max: 999999 }), (tmdbId) => {
    ...
    describe('Property 8: TBA episodes have null airDate/title/overview', () => {
        it('TBA episode fields are null when first_aired is null', () => {
            fc.assert(fc.property(fc.integer({ min: 1, max: 100 }), fc.integer({ min: 1000, max: 9999 }), (epNumber, traktEpId) => {
                const tbaEp = {
                    number: epNumber,
                    season: 1,
                    title: null,
    ...
    describe('Property 9: displayLanguage null skips TMDB calls', () => {
        it('TMDB call count is 0 when displayLanguage is null or empty', () => {
            fc.assert(fc.property(fc.oneof(fc.constant(null), fc.constant('')), (displayLanguage) => {
                let tmdbCallCount = 0;
    ...
    describe('Property 10: TMDB translation only written when different from Trakt', () => {
        it('translatedName is null when titles are equal, non-null when different', () => {
            fc.assert(fc.property(fc.string({ minLength: 1, maxLength: 50 }), fc.string({ minLength: 1, maxLength: 50 }), (traktTitle, tmdbTitle) => {
        ...
        it('translatedOverview is null when overviews are equal, non-null when different', () => {
            fc.assert(fc.property(fc.string({ minLength: 1, maxLength: 200 }), fc.string({ minLength: 1, maxLength: 200 }), (traktOverview, tmdbOverview) => {
    ...
    describe('Property 11: TMDB failure does not interrupt sync', () => {
        it('sync continues and returns showId even when TMDB throws', async () => {
            await fc.assert(fc.asyncProperty(fc.integer({ min: 1, max: 999999 }), async (tmdbId) => {
                ...
                try {
                    ...
                    console.warn('[test] TMDB failed:', e.message);
                    tmdbFailed = true;
    ...
    describe('Property 12: show with null ids.tmdb throws Missing TMDB id error', () => {
        it('throws error containing "Missing TMDB id" when ids.tmdb is null', () => {
            fc.assert(fc.property(fc.integer({ min: 1, max: 999999 }), (traktId) => {
                const traktDetail = makeTraktDetail(null, traktId);
                // Simulate the guard in upsertShowFromTrakt
                const throwIfMissingTmdb = (detail) => {
                    if (!detail.ids.tmdb) {
    ...
    describe('Property 5: Season 0 is not filtered in getSeasons result', () => {
        it('season 0 passes through without filtering', () => {
            fc.assert(fc.property(fc.array(fc.integer({ min: 1, max: 10 }), { minLength: 0, maxLength: 5 }), (extraNums) => {
    ...
    describe('Property 6: TBA episodes are not filtered', () => {
        it('episodes with null first_aired pass through without filtering', () => {
            fc.assert(fc.property(fc.array(fc.integer({ min: 2, max: 20 }), { minLength: 0, maxLength: 5 }), (airedNums) => {
