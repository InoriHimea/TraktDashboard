...
import { recalcShowProgress } from '../services/sync.js';
export const showRoutes = new Hono();
...
showRoutes.get('/progress', async (c) => {
    ...
    const whereClause = and(eq(userShowProgress.userId, userId), search ? like(shows.title, `%${search}%`) : undefined, filter === 'watching' ? eq(userShowProgress.completed, false) : undefined, filter === 'completed' ? eq(userShowProgress.completed, true) : undefined);
...
showRoutes.get('/:showId/episodes/:season/:episode', async (c) => {
    ...
    const [ep] = await db.select().from(episodes).where(and(eq(episodes.showId, showId), eq(episodes.seasonNumber, season), eq(episodes.episodeNumber, episode)));
    if (!ep)
    ...
    const { directors } = await getTmdbEpisodeDetail(show.tmdbId, season, episode, show.displayLanguage ?? undefined, userId);
...
showRoutes.get('/:showId/episodes/:season/:episode/history', async (c) => {
    ...
    const history = await db.select({
        ...
        .from(watchHistory)
        .innerJoin(episodes, eq(watchHistory.episodeId, episodes.id))
        .where(and(eq(watchHistory.userId, userId), eq(episodes.showId, showId), eq(episodes.seasonNumber, season), eq(episodes.episodeNumber, episode)))
...
showRoutes.delete('/:showId/history/:historyId', async (c) => {
    ...
    const [record] = await db.select()
        .from(watchHistory)
        .innerJoin(episodes, eq(watchHistory.episodeId, episodes.id))
        .where(and(eq(watchHistory.id, historyId), eq(watchHistory.userId, userId), eq(episodes.showId, showId)));
    if (!record)
