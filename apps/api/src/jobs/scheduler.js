...
async function getUserSyncInterval(userId) {
    try {
        const db = getDb();
        const [row] = await db
        ...
        if (row?.syncIntervalMinutes)
            return row.syncIntervalMinutes;
    }
    catch {
...
export async function registerUserSyncJob(userId) {
...
export async function enqueueSyncNow(userId) {
    const queue = getSyncQueue();
    await queue.add("incremental-sync", { userId }, {
        priority: 1,
        removeOnComplete: 5,
