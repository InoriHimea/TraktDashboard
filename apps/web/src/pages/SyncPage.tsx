      ...
      >
        {isLoading ? (
          <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
            ...
            <span style={{ fontSize: '14px', color: 'var(--color-text-secondary)' }}>正在加载同步状态…</span>
          </div>
        ) : isRunning ? (
          /* ── Running state ── */
                ...
                <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: '16px' }}>
                  <div style={{ flex: 1 }}>
                    ...
                    <p style={{ fontSize: '12px', color: 'var(--color-text-muted)', lineHeight: 1.6 }}>
                      仅同步上次以来的新观看记录，速度快。同时会拉取 TMDB 的本地化标题（根据设置中的显示语言）。
                    </p>
                  </div>
                  <motion.button
                    onClick={() => triggerSync()}
                    ...
                    style={{
                      ...
                      padding: '9px 18px',
                      borderRadius: 'var(--radius-md)',
                      background: anyPending || isRunning ? 'var(--color-surface-3)' : '#7c6af7',
                      color: anyPending || isRunning ? 'var(--color-text-muted)' : '#fff',
                      fontSize: '13px',
                      fontWeight: 600,
                      border: 'none',
                      ...
                      boxShadow: anyPending || isRunning ? 'none' : '0 2px 12px rgba(124,106,247,0.4)',
                    }}
                  >
              ...
              <div style={{
                ...
                <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: '16px' }}>
                  <div style={{ flex: 1 }}>
                    ...
                    <p style={{ fontSize: '12px', color: 'var(--color-text-muted)', lineHeight: 1.6 }}>
                      重新同步全部历史记录，并强制刷新所有剧集的 TMDB 元数据（含本地化标题、简介）。耗时较长，适合首次使用或修复数据。
                    </p>
                  </div>
                  <motion.button
                    onClick={() => triggerFull()}
                    ...
                    style={{
                      ...
                      padding: '9px 18px',
                      borderRadius: 'var(--radius-md)',
                      background: anyPending || isRunning ? 'var(--color-surface-3)' : '#0ea5e9',
                      color: anyPending || isRunning ? 'var(--color-text-muted)' : '#fff',
                      fontSize: '13px',
                      fontWeight: 600,
                      border: anyPending || isRunning ? '1px solid transparent' : '1px solid #38bdf840',
                      cursor: anyPending || isRunning ? 'not-allowed' : 'pointer',
                      flexShrink: 0,
                      transition: 'background 0.15s',
                      boxShadow: anyPending || isRunning ? 'none' : '0 2px 12px rgba(14,165,233,0.35)',
                    }}
                  >
                    <Database size={14} className={fullSyncing ? 'animate-spin' : ''} />
