...
vi.mock('../../hooks', () => ({
    useShowDetail: (...args) => mockUseShowDetail(...args),
}));
// Import after mocks
...
describe('ShowDetailPage', () => {
    ...
    it('P2: rendered SeasonTab count equals seasons.length', () => {
        fc.assert(fc.property(fc.array(fc.integer({ min: 1, max: 20 }), { minLength: 1, maxLength: 8 }), (seasonNums) => {
            // Deduplicate season numbers
            const unique = [...new Set(seasonNums)];
            const seasons = unique.map(n => makeSeason(n));
            mockUseShowDetail.mockReturnValue({ data: makeProgress(seasons), isLoading: false });
            const { container, unmount } = render(_jsx(ShowDetailPage, {}));
            ...
            const seasonTabCount = Array.from(seasonLabels).filter(el => el.textContent?.match(/^第 \d+ 季$/)).length;
            expect(seasonTabCount).toBe(unique.length);
            unmount();
        }));
    });
});
//# sourceMappingURL=ShowDetailPage.test.js.map