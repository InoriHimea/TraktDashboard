        ...
        <div className="flex gap-8 items-start animate-pulse">
          ...
          <div className="flex-1 min-w-0 flex flex-col gap-6">
            ...
            <div className="flex gap-3">
              {[90, 90, 90, 90].map((w, i) => (
                <div key={i} className="h-[130px] rounded-xl bg-white/[0.03]" style={{ width: w }} />
              ))}
            </div>
          </div>
          {/* 侧栏 */}
    ...
    <div className="min-h-screen bg-[var(--color-bg)] text-[var(--color-text)]">
      <div style={{ width: '100%', padding: '24px 40px' }}>
        ...
        <div ref={episodesRef} style={{ marginTop: '40px', paddingTop: '32px', borderTop: '1px solid var(--color-border-subtle)' }}>
          ...
          {/* Breadcrumb */}
          <div style={{ display: 'flex', alignItems: 'center', gap: '6px', marginBottom: '24px', fontSize: '14px' }}>
            <span style={{ fontWeight: 600, color: 'var(--color-text)' }}>Seasons</span>
            <span style={{ color: 'var(--color-text-muted)' }}>/</span>
            <span style={{ color: 'var(--color-text-muted)' }}>{currentSeasonNumber === 0 ? 'Specials' : `Season ${currentSeasonNumber}`}</span>
          </div>

          {/* 季海报横排 — 靠左 */}
          {seasons.length > 0 && (
            <div
              style={{ display: 'flex', gap: '24px', marginBottom: '40px', overflowX: 'auto', paddingBottom: '8px', scrollbarWidth: 'none' }}
              role="tablist"
              aria-label="选择季度"
            >
              {seasons.map(s => (
          ...
          {/* 剧集横向滚动列表 */}
          <div style={{ paddingTop: '24px', borderTop: '1px solid var(--color-border-subtle)' }}>
