...
describe('EpisodeCard', () => {
  it('always renders Tv2 placeholder (no stillPath)', () => {
    const { container } = render(
      <EpisodeCard episode={makeEpisode()} index={0} seasonNumber={1} />
    )
  ...
  it('renders 未播出 badge when aired=false', () => {
    render(<EpisodeCard episode={makeEpisode({ aired: false })} index={0} seasonNumber={1} />)
    expect(screen.getByText('未播出')).toBeTruthy()
  })

  it('does not render 未播出 badge when aired=true', () => {
    render(<EpisodeCard episode={makeEpisode({ aired: true })} index={0} seasonNumber={1} />)
      ...
      fc.property(
        ...
        (seasonNumber, episodeNumber) => {
          ...
          const { unmount } = render(
            <EpisodeCard
              episode={makeEpisode({ episodeNumber })}
              index={0}
      ...
      fc.property(
        ...
        (title) => {
          ...
          const { container, unmount } = render(
            <EpisodeCard episode={ep} index={0} seasonNumber={1} />
          )
  ...
  it('P9: unaired card has reduced opacity and 未播出 text', () => {
    fc.assert(
      fc.property(
        fc.record({ episodeNumber: fc.nat({ max: 20 }) }),
        ({ episodeNumber }) => {
          const { container, unmount } = render(
            <EpisodeCard
              episode={makeEpisode({ aired: false, watched: false, episodeNumber })}
              index={0}
              seasonNumber={1}
            />
          )
  ...
  it('P10: watch-count indicator not shown when watchCount <= 1', () => {
    ...
    const { container } = render(
      <EpisodeCard episode={makeEpisode({ watched: true })} index={0} seasonNumber={1} />
    )
