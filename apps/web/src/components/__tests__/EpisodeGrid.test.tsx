...
function makeEpisode(n: number): EpisodeProgress {
  return {
    episodeId: n,
    seasonNumber: 1,
    episodeNumber: n,
...
describe('EpisodeGrid', () => {
  it('renders correct number of episode cards', () => {
    ...
    const { container } = render(<EpisodeGrid episodes={episodes} seasonNumber={1} />)
  ...
  it('renders empty grid for empty episodes array', () => {
    const { container } = render(<EpisodeGrid episodes={[]} seasonNumber={1} />)
    const cards = container.querySelectorAll('[tabindex="0"]')
  ...
  it('P5: rendered card count equals episodes.length', () => {
    fc.assert(
      fc.property(fc.array(fc.nat({ max: 50 }), { maxLength: 15 }), (nums) => {
        const episodes = nums.map((n, i) => makeEpisode(i + 1))
        const { container, unmount } = render(
          <EpisodeGrid episodes={episodes} seasonNumber={1} />
        )
        const cards = container.querySelectorAll('[tabindex="0"]')
