...
describe('Property 5: poster error shows placeholder', () => {
    it('renders Tv2 placeholder when img fires onError for any valid data', () => {
        fc.assert(fc.property(fc.integer({ min: 1, max: 20 }), fc.integer({ min: 1, max: 20 }), (seasonNumber, episodeNumber) => {
            const data = makeData({ episode: { seasonNumber, episodeNumber, title: 'Ep' } });
            const { unmount } = render(_jsx(NowPlayingPopup, { data: data, isLoading: false, isOpen: true, onClose: () => { } }));
...
describe('Property 6: popup visible iff isOpen=true', () => {
    it('popup is in DOM when isOpen=true, absent when isOpen=false', () => {
        fc.assert(fc.property(fc.boolean(), (isOpen) => {
            const data = makeData();
            const { unmount } = render(_jsx(NowPlayingPopup, { data: data, isLoading: false, isOpen: isOpen, onClose: () => { } }));
...
//# sourceMappingURL=NowPlayingPopup.property.test.js.map