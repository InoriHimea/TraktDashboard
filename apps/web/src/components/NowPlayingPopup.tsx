...
function Skeleton() {
  return (
    <div data-testid="now-playing-skeleton" style={{ display: 'flex', gap: '12px', padding: '16px' }}>
      <div style={{
        width: '56px', height: '80px', borderRadius: '6px',
        background: 'var(--color-surface-3)', flexShrink: 0,
        animation: 'pulse 1.5s ease-in-out infinite',
      }} />
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: '8px', paddingTop: '4px' }}>
        <div style={{ height: '10px', width: '60%', borderRadius: '4px', background: 'var(--color-surface-3)' }} />
        <div style={{ height: '10px', width: '80%', borderRadius: '4px', background: 'var(--color-surface-3)' }} />
        <div style={{ height: '10px', width: '40%', borderRadius: '4px', background: 'var(--color-surface-3)' }} />
      </div>
    </div>
  )
}
...
export function NowPlayingPopup({ data, isLoading, isOpen, onClose }: NowPlayingPopupProps) {
  const cardRef = useRef<HTMLDivElement>(null)
  const [posterError, setPosterError] = useState(false)

  // Reset poster error when data changes
  useEffect(() => { setPosterError(false) }, [data?.show.posterPath])
  ...
  const remainingMinutes = data ? computeRemainingMinutes(data.expiresAt) : 0
      ...
      {isOpen && (
        ...
        >
          ...
          <div style={{
            ...
            <span style={{ fontSize: '12px', fontWeight: 600, color: 'var(--color-text)', letterSpacing: '0.02em' }}>
              Now Playing
            </span>
          </div>
            ...
            <div style={{ padding: '14px', display: 'flex', gap: '12px' }}>
              ...
              <div style={{
                ...
                {data.show.posterPath && !posterError ? (
                  <img
                    src={`https://image.tmdb.org/t/p/w92${data.show.posterPath}`}
                    alt={data.show.title}
                    onError={() => setPosterError(true)}
                    style={{ width: '100%', height: '100%', objectFit: 'cover' }}
                  />
                ) : (
                  <Tv2 size={22} style={{ color: 'var(--color-text-muted)' }} data-testid="poster-placeholder" />
                )}
              </div>
              ...
              <div style={{ flex: 1, minWidth: 0, display: 'flex', flexDirection: 'column', gap: '4px' }}>
                <span style={{ fontSize: '11px', color: 'var(--color-accent)', fontWeight: 600 }}>
                  {formatSeasonEpisode(data.episode.seasonNumber, data.episode.episodeNumber)}
                </span>
                <p style={{
                ...
                {data.runtime != null && (
                  <div style={{
                    marginTop: '6px', height: '3px', borderRadius: '999px',
