...
| N3-T02 | **api 侧已完成，web 侧进行中**— 提升覆盖率阈值，2026-07-07 重定基为诚实 ratchet；api 侧（plan-20260712~715.md）已于 2026-07-15 收官：**85.8%/72.9%/87.3%/87.6%**（stmts/branch/funcs/lines），阈值 85/72/87/87，795 个测试。2026-07-15b（plan-20260715b.md，web 侧，13 个批次，起点 25.45%/21.9%/18.24%/26.26%，缺口分散在 35+ 个组件/页面约 15700 行）：批次 1-9 已完成——基础设施层（api.ts/hooks/push.ts）、小型展示组件、`MediaListPage`+Movies/TVShows/WatchlistPage、stats 小组件 8 个、`JellyfinStatsPage` 全部、中型交互组件 A（EpisodeGrid/UpNextBanner/WatchHistoryPanel/SearchModal）、中型组件/页面 B（TopNav/DiscoverPage/CalendarPage）。web 整体到 **64.7%/53.1%/64.5%/64.9%**，阈值 64/53/64/64，540 个测试。常见踩坑（详见各 plan 文档"踩坑"记录）：react-query 裸 mutationFn 透传额外参数；framer-motion/AnimatePresence 的动画在 jsdom 不可靠（改断言副作用而非 DOM 状态）；`t()`/dayjs locale 未 mock 时解析真实 zh-CN 或默认 locale；toast 需要同渲染 `<Toaster/>`；recharts `ResponsiveContainer` 在 jsdom 宽高恒 0 不渲染图表 DOM；jsdom 回读内联颜色会重新序列化、且未实现 `scrollIntoView`（需手动 stub）；lucide 图标内部注册名可能与导出名不同（需读源码确认 class）；`vi.useFakeTimers()` 必须带 `shouldAdvanceTime:true` 才能与 `waitFor()` 共存；模块级 mock 需手动 `mockClear()`；断言内部排序逻辑时 fixture 要故意乱序声明；多条目场景下图片回退链测试要用单条目 fixture 隔离。批次 10（大型组件/页面 A：NowPlayingPopup/EpisodeInfoCard/LoginPage）起待续 | 中     |
