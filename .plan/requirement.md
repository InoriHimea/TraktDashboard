...
| N3-T02 | **api 侧已完成，web 侧进行中**— 提升覆盖率阈值，2026-07-07 重定基为诚实 ratchet；api 侧（plan-20260712~715.md）已于 2026-07-15 收官：**85.8%/72.9%/87.3%/87.6%**（stmts/branch/funcs/lines），阈值 85/72/87/87，795 个测试。2026-07-15b（plan-20260715b.md，web 侧，13 个批次，起点 25.45%/21.9%/18.24%/26.26%，缺口分散在 35+ 个组件/页面约 15700 行）：批次 1-11 已完成（基础设施层→小型组件→stats/Jellyfin 小组件→中型交互组件→中/大型页面，详见 plan 文档各批次"踩坑"记录）。web 整体到 **75.9%/69.3%/73.7%/76.6%**，阈值 75/69/73/76，627 个测试。高频踩坑速查：react-query 裸 mutationFn 透传额外参数；framer-motion/AnimatePresence 动画在 jsdom 不可靠（`mode="wait"` 会导致新 tab 内容永不挂载，需整文件 mock 掉 framer-motion；关闭动画不清场则需按各自标题 scoping 点击范围）；`t()`/dayjs locale 未 mock 时解析真实 zh-CN/默认 locale，断言前先查实际翻译值而非凭印象；toast 需要同渲染 `<Toaster/>`；recharts 在 jsdom 不渲染图表 DOM；jsdom 回读内联颜色会重新序列化、未实现 `scrollIntoView`；lucide 图标内部注册名可能与导出名不同；`vi.useFakeTimers()` 需配 `shouldAdvanceTime:true` 才能与 `waitFor()`/`setInterval回调` 共存；模块级 mock 需手动 `mockClear()`；`aria-label` 会静默覆盖可访问名。批次 12（超大型页面 A：ListsPage/CollectionPage）起待续 | 中     |
