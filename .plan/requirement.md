...
| N3-T02 | **提升覆盖率阈值**（api 侧阶段性完成，web 侧待续）— 曾达成 40%/28%，N4/N5 未测新代码使实际衰减至 ~24%/~23%，2026-07-07 重定基为诚实 ratchet；2026-07-12 两批：collection/lists/stats/ratings/notes/auth-middleware/system + trakt/discover/search/img/auth 路由测试，api 回升至 41.0%/33.5%/38.2%/42.1%；web 端 queryKeys/StatCard 等小组件 + notes-lists-ratings hooks，回升至 25.5%/21.9%/18.2%/26.3%；2026-07-13 第三批（plan-20260713.md）：jellyfin 服务层 99.4%+路由层 97.7%、backup 四云 provider 97.0%（服务层）+ dumpDatabase/restoreDatabase（child_process 层）+ routes/backup.ts 74.9%（含全仓库最危险的 `POST /restore` 端点），api 回升至 65.3%/53.9%/65.9%/65.9%；2026-07-14 第四批（plan-20260714.md）：lib/push.ts 100%、services/tmdb.ts 98.8%、jobs/scheduler.ts 93.5%（mock bullmq Queue/Worker + ioredis，捕获 Worker 处理回调驱动五种 job.name 分支），api 整体到 **69.6%/57.1%/71.0%/70.5%**（stmts/branch/funcs/lines），阈值同步上调至 67/55/69/68；2026-07-15（plan-20260715.md）：将剩余四个大文件（services/sync.ts 2328 行、services/trakt.ts 1058 行、routes/shows.ts 808 行、routes/history.ts 409 行，合计约 4600 行）拆解为 10 个可独立执行验证的批次；批次 1（history.ts POST /import，35.4%→98.1%）+ 批次 2-3（shows.ts 12 个路由，23.1%→92.4%，含 fire-and-forget Jellyfin 自动删除 IIFE 的四门槛条件验证）+ 批次 4-5（services/trakt.ts ~50 个客户端方法，35.5%→93.1%，含分页循环/watching类型过滤/11个变更方法body分桶/4个缓存方法——过程中勘误：getShowDetail/getSeasons/getEpisodes 并无 tmdb.ts 式的 grace-window 降级，fetch 失败直接抛出，只有 getEpisodeRating 才有 stale-or-null 降级）+ 批次 6（services/sync.ts 小型纯函数/进度重算：getSyncStatus/getPostResetWatchedEpisodeIds/computeWatchedEpisodes/recalcShowProgress/recalcMovieProgress/resetStaleRunningSyncs，25.3%→28.6%，涨幅有限属预期——该文件最大的两个函数 forceSyncShow ~673 行、syncMovies ~386 行仍未测）+ 批次 7（syncWatchlist tmdb→trakt→imdb 级联匹配+清理删除/syncRatings 批量 inArray 解析/syncUserCollection show+episode+movie 的 insert-vs-update 分支，28.6%→39.9%）+ 批次 8（syncMovies：TMDB元数据解析+语言回退本地化/tmdbId有无两条upsert路径/摘要-only跳过收尾recalc/陈旧trakt历史全量清理，39.9%→42.3%）+ 批次 10（forceSyncShow，提前于批次 9 执行——**勘误**：该函数本身只有 ~16 行，"全仓库最大函数 ~673 行"是早前把它与后面 syncSingleShow/upsertShowFromTrakt/syncEpisodeProgress/findOrCreateEpisode 等同级私有辅助函数之间的行距误算成单个函数体；这批用按表引用分派的 DB mock 覆盖了这条 triggerFullSync/triggerIncrementalSync 也共享的调用链，42.3%→58.1%，涨幅远超预期）已完成，api 整体到 **83.0%/71.6%/85.0%/84.5%**（stmts/branch/funcs/lines），阈值同步上调至 82/71/85/84；剩余批次 9（triggerFullSync/triggerIncrementalSync 的编排逻辑——并发锁/pLimit循环/失败重试/游标回退/sync state状态机，共享的单剧同步逻辑已被批次10覆盖）待续；web 侧多数页面组件仍 0%（未动） | 中     |
