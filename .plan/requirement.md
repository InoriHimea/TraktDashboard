...
| N3-T02 | ✅ **api 侧全部完成**（web 侧待续）— 提升覆盖率阈值，曾达成 40%/28%，N4/N5 未测新代码使实际衰减至 ~24%/~23%，2026-07-07 重定基为诚实 ratchet；2026-07-12/13/14 三批（plan-20260712/13/14.md）逐步回升到 69.6%/57.1%/71.0%/70.5%（stmts/branch/funcs/lines）；2026-07-15（plan-20260715.md，10 个批次全部完成，详见该文件收尾小结）：拆解并清零最后四个大文件的覆盖率缺口——`routes/history.ts` 35.4%→98.1%、`routes/shows.ts` 23.1%→92.4%、`services/trakt.ts` 35.5%→93.1%、`services/sync.ts`（全仓库最复杂文件）25.3%→75.7%；api 整体最终到 **85.8%/72.9%/87.3%/87.6%**（stmts/branch/funcs/lines），阈值 ratchet 至 85/72/87/87；新增 8 个测试文件、158 个测试（654→795）。过程中勘误：`services/trakt.ts` 的 `getShowDetail`/`getSeasons`/`getEpisodes` 无 grace-window 降级（与 tmdb.ts 不同）；`forceSyncShow` 本身仅 ~16 行（早前"全仓库最大函数 ~673 行"的估算误把相邻私有辅助函数的行距算作其函数体）。web 侧多数页面组件仍 0%，是 N3-T02 下一个可选方向（暂无排期） | 中     |
