...
| N3-T02 | **api 侧已完成，web 侧进行中**— 提升覆盖率阈值，曾达成 40%/28%，N4/N5 未测新代码使实际衰减至 ~24%/~23%，2026-07-07 重定基为诚实 ratchet；2026-07-12/13/14/15 四批（plan-20260712/13/14/15.md）api 侧从 22%逐步回升并于 2026-07-15 收官：拆解清零最后四个大文件（`routes/history.ts`/`routes/shows.ts`/`services/trakt.ts`/`services/sync.ts`），api 整体到 **85.8%/72.9%/87.3%/87.6%**（stmts/branch/funcs/lines），阈值 85/72/87/87，795 个测试。2026-07-15b（plan-20260715b.md，web 侧，13 个批次）：起点 25.45%/21.9%/18.24%/26.26%，缺口分散在 35+ 个组件/页面（约 15700 行），按层级分批。已完成：批次 1-3 基础设施层（`lib/api.ts` 表驱动全量方法 11.3%→98.7%、`hooks/index.ts` 剩余 63 个 hook 17.1%→99.7%、`lib/push.ts` 浏览器端订阅流程 1.8%→100%）；批次 4 小型展示组件 8 个（55.55%-100%）；批次 5 `MediaListPage`（100%）+`MoviesPage`/`TVShowsPage`/`WatchlistPage`；批次 6 stats 小组件 8 个（`MediaComposition`/`WatchHeatmap`/`RecentActivity`/`TraktStats`/`ScreenTime` 100%，`RatingDistribution`/`WatchPatterns`/`ActivityChart` 受限于 recharts 在 jsdom 不渲染图表 DOM 而较低）；批次 7 `JellyfinStatsPage` 全部 5 个文件均 100%。web 整体到 **53.9%/39.6%/55.8%/54.1%**，阈值 53/39/55/54，467 个测试。踩坑集锦：react-query 对裸函数引用 mutationFn 透传额外参数；framer-motion 的 animate prop 在 jsdom 不确定性反映到 style；`t()` 未 mock 解析出真实 zh-CN 文本，断言需用实际中文或故意用不存在的 key；toast() 需要同时渲染 `<Toaster/>` 才能断言可见文案；recharts 的 `ResponsiveContainer` 在 jsdom 下宽高恒为 0，不渲染任何图表 DOM（但子节点构造仍会执行，覆盖率仍受益）；jsdom 回读内联 rgba/hex 颜色会重新序列化（alpha=1 时丢弃 alpha，hex 转 rgb()）；dayjs `fromNow()` 需固定系统时间且渲染语言取决于该文件是否自己调用过 `dayjs.locale()`；lucide 图标内部注册名可能与导出名不同（`Tv2`→`lucide-tv-minimal`）。批次 8（中型组件 A：EpisodeGrid/UpNextBanner/WatchHistoryPanel/SearchModal）起待续 | 中     |
