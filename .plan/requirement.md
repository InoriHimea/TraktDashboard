...
| N2-T17 | ✅ **已完成**（2026-07-18，plan-20260718.md，2 个批次）：Trakt 观看历史重复记录审计与清理——修复外部 Jellyfin 插件同步 bug 造成的 Trakt 历史重复数据。审计完全跑本地 SQL（`watch_history.trakt_play_id` 已是 Trakt 历史的逐条镜像，无需重新调用 Trakt API），按 `trakt_play_id IS NOT NULL` 过滤（不能用 `source='trakt'`——剧集同步路径从未显式设置该字段）；链式聚类区分"疑似 bug 重复"（间隔短）与"真实重看"（间隔长，保留）；用户确认后调用 Trakt `POST /sync/history/remove` 永久删除，本地镜像同步清理；顺带修复既有单条历史删除路由只删本地、不调 Trakt 的潜在 bug。新页面 `/history/duplicates`，54 个新/改测试（api 28 + web 26），未新增数据库迁移 | 中     |
...
| N3-T02 | ✅ **已完成**（api 侧+web 侧均收官，2026-07-15）— 提升覆盖率阈值，2026-07-07 重定基为诚实 ratchet。api 侧（plan-20260712~715.md）：**85.8%/72.9%/87.3%/87.6%**（stmts/branch/funcs/lines），阈值 85/72/87/87，795 个测试。web 侧（plan-20260715b.md，13 个批次，起点 25.45%/21.9%/18.24%/26.26%）：全部批次完成（基础设施层→小型组件→stats/Jellyfin 小组件→中型交互组件→中/大型页面→超大型页面，详见 plan 文档各批次"踩坑"记录），终点 **86.2%/81.3%/83.1%/87.6%**，阈值 86/81/83/87，765 个测试。高频踩坑速查（供后续任何 vitest+jsdom 组件测试参考）：react-query 裸 mutationFn 透传额外参数；framer-motion/AnimatePresence 动画在 jsdom 不可靠（`mode="wait"` 会导致新 tab 内容永不挂载，需整文件 mock 掉 framer-motion；关闭动画不清场则需按各自标题 scoping 点击范围；悬停触发的按钮容器常靠 opacity 切换可见性而非条件挂载，同屏多实例时同 class 查询要先按各自唯一文案 scoping）；`t()`/dayjs locale 未 mock 时解析真实 zh-CN/默认 locale，断言前先查实际翻译值而非凭印象；toast 需要同渲染 `<Toaster/>`；recharts 在 jsdom 不渲染图表 DOM；jsdom 回读内联颜色会重新序列化、未实现 `scrollIntoView`；lucide 图标内部注册名可能与导出名不同；`vi.useFakeTimers()` 需配 `shouldAdvanceTime:true` 才能与 `waitFor()`/`setInterval回调` 共存（`setInterval(async () => ...)` 轮询需要 `vi.advanceTimersByTimeAsync`）；模块级 mock 需手动 `mockClear()`；`aria-label` 会静默覆盖可访问名；同翻译 key 复用于多处文案时优先用独有的标题/副标题文字 scoping 而非裸文本查询 | 中     |
