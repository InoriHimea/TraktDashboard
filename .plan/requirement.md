...
| N3-T02 | **api 侧已完成，web 侧启动**— 提升覆盖率阈值，曾达成 40%/28%，N4/N5 未测新代码使实际衰减至 ~24%/~23%，2026-07-07 重定基为诚实 ratchet；2026-07-12/13/14 三批（plan-20260712/13/14.md）逐步回升；2026-07-15（plan-20260715.md，10 个批次全部完成，详见该文件收尾小结）：✅ api 侧收官——拆解并清零最后四个大文件的覆盖率缺口（`routes/history.ts` 35.4%→98.1%、`routes/shows.ts` 23.1%→92.4%、`services/trakt.ts` 35.5%→93.1%、`services/sync.ts` 25.3%→75.7%），api 整体到 **85.8%/72.9%/87.3%/87.6%**（stmts/branch/funcs/lines），阈值 ratchet 至 85/72/87/87；新增 8 个测试文件、158 个测试（654→795）。过程中勘误：`services/trakt.ts` 的 `getShowDetail`/`getSeasons`/`getEpisodes` 无 grace-window 降级；`forceSyncShow` 本身仅 ~16 行（早前"全仓库最大函数 ~673 行"的估算有误）。2026-07-15b（plan-20260715b.md，启动，13 个批次）：web 侧起点 25.45%/21.9%/18.24%/26.26%，缺口分散在 35+ 个组件/页面（约 15700 行原始代码），非集中于少数大文件——按层级分批：基础设施层（`lib/api.ts` 586 行 11.3%、`hooks/index.ts` 896 行 76 个 hook 仅 13 个已测、`lib/push.ts` 137 行 1.8%）优先，其后小型组件→stats/JellyfinStatsPage→中大型页面→超大型页面（`ListsPage`/`CollectionPage`/`settings/BackupTab.tsx` 1488 行）。批次 1（`lib/api.ts`，19 个域命名空间/约 90 个方法表驱动覆盖 GET query 拼接+写操作 method/body，含 `history.export` 不走 fetch 的特例）+ 批次 2（`hooks/index.ts` 剩余 63 个 hook：34 个查询表驱动+23 个变更表驱动+特殊逻辑 hook 单独验证，含 useTriggerSync/useTriggerFullSync 裸函数引用导致 react-query 透传额外参数的踩坑）+ 批次 3（`lib/push.ts` 浏览器端 Web Push 订阅流程：支持性检测/enablePush 全部9条分支含并发标签页竞态与SW退化场景/disablePush，1.8%→100%）已完成，**基础设施层（批次1-3）全部完成**，web 整体到 **41.0%/26.8%/45.3%/41.5%**，阈值上调至 41/26/45/41；+ 批次 4（8 个小型展示组件 Layout/ShowCard/SeasonTab/ProgressBar/TraktProgressBar/DateTimePickerModal/SlidingPanel/EpisodeSeasonStrip，55.55%-100%——踩坑：framer-motion 的 animate prop 不会在 jsdom 里确定性反映到 style，改为验证不报错渲染而非动画样式值）已完成，web 整体到 **44.2%/31.0%/47.5%/44.7%**，阈值上调至 44/31/47/44；批次 5（小型页面）起待续 | 中     |
