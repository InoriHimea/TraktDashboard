...
| N5-T10 | **10.11 适配：Phase 2 遇 500 自动 defer 7 天并在历史标注"文件可能已删（jellyfin#16975）"** — 避免幽灵导致的每日重试死循环；12.0 后可移除                                                                                                    | 高     |
| N5-T02 | **队列条目"立即删除"按钮** — 确认式，不等次日定时任务                                                                                                                                                                                       | 中     |
| N5-T03 | **删除前 Web Push 提醒** — 条目首次入队时推送"X 将于明天删除"（需 VAPID 已配置）                                                                                                                                                            | 中     |
| N5-T04 | **电影自动删除** — 队列表 movie_id 已预留；判定规则待定（如已看完 + 满 N 天）                                                                                                                                                               | 中     |
| N5-T05 | **修复 upsertRepeatableJob 去重** — getRepeatableJobs() 无 .id 字段导致重复注册（airing-reminders/cleanup-cache 各×2）                                                                                                                      | 高     |
| N5-T06 | **清理死代码 SyncPage.tsx**（36K，/sync 已重定向设置页）                                                                                                                                                                                    | 低     |
| N5-T07 | **设置页拆分页签** — 单文件 194K、单列长滚动，拆 常规/Jellyfin/通知/备份                                                                                                                                                                    | 低     |
| N5-T08 | **显示语言改下拉框** — 替代裸 BCP-47 文本输入                                                                                                                                                                                               | 低     |
| N5-T09 | **完成或放弃 N1-T02 RPC 迁移** — 手写 api.ts 与 typed rpc 并存的现状收敛                                                                                                                                                                    | 低     |
