...
| N2-T17 | 🚧 **进行中**：Trakt 观看历史重复记录审计与清理（plan-20260718.md，2 个批次）——修复外部 Jellyfin 插件同步 bug 造成的 Trakt 历史重复数据，审计+人工确认+调用 Trakt `/sync/history/remove` 永久删除；顺带修复既有单条历史删除路由只删本地、不调 Trakt 的潜在 bug | 中     |
